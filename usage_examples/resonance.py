...
diameters = 1.33 + np.linspace(0, 0.3, 5)
scatter_phases = []
scatter_intensities = []
camera_intensities = []
...
for d in diameters:
    params = DesignParams(scat_mat="gold", diameter=40, n_medium=d)
    num = 100
    ...
    camera_intensities.append(center_intensity_scat)
    interference_intensities.append(center_intensity_if)
...
for intensity, n in zip(camera_intensities, diameters):
    ax[1].plot(wavelens, intensity/np.max(intensity), label=f'n={n:.2f}')
